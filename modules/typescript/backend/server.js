...
// TypeScript Lessons API
app.get('/api/typescript/lessons', (req, res) => {
  try {
    const lessons = readJsonFile('lessons.json');
    const result = applyQueryParams(lessons, req);
    res.json(result);
  } catch (error) {
    console.error('Error fetching TypeScript lessons:', error);
    res.status(500).json({ error: 'Failed to fetch TypeScript lessons' });
  }
});

// TypeScript Interview Questions API
app.get('/api/typescript/questions', (req, res) => {
  try {
    const questions = readJsonFile('questions.json');
    const result = applyQueryParams(questions, req);
    res.json(result);
  } catch (error) {
    console.error('Error fetching TypeScript questions:', error);
    res.status(500).json({ error: 'Failed to fetch TypeScript questions' });
  }
});

// Submit answer for TypeScript questions
app.post('/api/typescript/answer', (req, res) => {
  try {
    ...
    console.error('Error validating TypeScript answer:', error);
