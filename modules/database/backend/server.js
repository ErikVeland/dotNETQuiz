...
// Database Lessons API
app.get('/api/database/lessons', (req, res) => {
  try {
    const lessons = readJsonFile('lessons.json');
    const result = applyQueryParams(lessons, req);
    res.json(result);
  } catch (error) {
    console.error('Error fetching Database lessons:', error);
    res.status(500).json({ error: 'Failed to fetch Database lessons' });
  }
});

// Database Interview Questions API
app.get('/api/database/questions', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch Database questions' });
  }
});

// Submit answer for Database questions
app.post('/api/database/answer', (req, res) => {
  try {
    ...
    console.error('Error validating Database answer:', error);
...
app.listen(PORT, () => {
  console.log(`Database backend server running on port ${PORT}`);
});